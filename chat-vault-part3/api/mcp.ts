...
/** Wrap res to log status and response body for debugging */
function wrapResponseToLog(
  res: ServerResponse,
): ServerResponse {
  ...
  res.end = function (
    ...
    const chunk = args[0];
    if (typeof chunk === "string") chunks.push(Buffer.from(chunk));
    else if (chunk && Buffer.isBuffer(chunk)) chunks.push(chunk);
    const body = Buffer.concat(chunks).toString("utf8");
    const preview = body.slice(0, 500) + (body.length > 500 ? "..." : "");

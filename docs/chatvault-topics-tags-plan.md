# ChatVault Topics / Tags Feature Plan
...
This plan adds **topics** (shown in the UI as **tags**) for organizing and filtering saved chats.
...
3. **Display tags on chats**: show topic chips on each row in the chats list.
4. **Manual tagging**: allow users to add/remove topics on a chat using the same combobox UX as the filter bar.
...
| --- | --- |
| Database / API | `topic` |
| UI copy | "Topic" or "Tag" (pick one label and use consistently; recommend **Topic** in UI) |
...
- **`source`** distinguishes auto-assigned vs user-edited tags (useful for analytics and future "don't overwrite manual tags" rules).
...
```ts
topicIds?: string[];   // filter: chats matching ANY selected topic (OR semantics)
topicMatch?: "any" | "all";  // optional, default "any"; defer "all" to v2 if needed
```
...
- Replace `chat_topics` where `source = 'manual'` OR replace entire set (v1: **replace entire set** — simpler UX when user edits tags on a row).
...
Optional future: `topics?: string[]` on manual save modal to pre-seed tags.
...
[ Topic filter: [chip] [chip] [type here... ▼]            ]
```
...
- Text search + topic filter compose: **both apply** (AND): query matches semantically AND chat has one of selected topics
...
Repeat topic chips + edit control under the title block (same component), so users can tag while reading a chat.
...
4. User adds second topic → OR filter shows chats tagged with either topic.
...
### Flow C: Manual tag on existing chat

1. User clicks tag area on row → combobox opens with current tags selected.
...
- [ ] Migration: `topics`, `chat_topics`
- [ ] Drizzle schema + relations
- [ ] `listTopics` tool
- [ ] Extend `loadMyChats` to return `topics` per chat + `availableTopics`
- [ ] Extend `loadMyChats` with `topicIds` filter
- [ ] Tests + list path regression
...
**Exit criteria:** Manual tag/edit works end-to-end.
...
| --- | --- | --- |
| 1 | Filter match: ANY vs ALL topics? | **ANY** for v1 |
| 2 | UI label: "Topic" or "Tag"? | **Topic** (matches domain model) |
| 3 | Replace vs merge on manual tag edit? | **Replace full set** from combobox state |
| 4 | Pre-seed tags on manual save modal? | Defer to v1.1 |
| 5 | Backfill topics for existing chats? | Optional offline script; not blocking launch |
...
- Manual tag edit completes in < 500 ms perceived (optimistic UI)
